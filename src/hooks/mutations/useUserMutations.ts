// hooks/mutations/useUserMutations.ts - 수정된 사용자 데이터 변경 훅
import {
  useMutation,
  useQueryClient,
  UseMutationResult,
...
> => {
  ...
  return useMutation({
    ...
    onSuccess: (user, variables) => {
      ...
      queryClient.setQueryData(["localUser"], user);
...
> => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // 서버 응답이 전체 사용자 정보를 반환하므로 그대로 사용
      return data.user as User;
    },
    onSuccess: (updatedUser, variables) => {
      ...
      queryClient.setQueryData(["localUser"], updatedUser);

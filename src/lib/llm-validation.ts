...
규칙:
1) **상황응답 평가** (원본이 상황/맥락을 제시하는 경우):
   - 원본이 "~하겠습니다", "~모르겠습니다", "~할 예정입니다" 등 상황 설명이면
   - STT가 그 상황에서 실제로 할 법한 구체적 발화/명령이면 approved
   - 예: 원본 "길을 몰라서 도움을 요청하겠습니다" → STT "○○역에서 △△역 가는 법 알려줘" (approved)
   
2) **따라읽기 평가** (원본이 직접적 발화/명령인 경우):
   - 원본이 구체적 명령이나 질문이면 STT도 거의 동일해야 함
   - 예: 원본 "자양역에서 경복궁역 가는 법" → STT도 유사한 교통 질문이어야 함
   
3) 판단 기준:
   - 원본에 "~겠습니다", "~모르겠다", "예정이다" 등이 있으면 → 상황응답 평가
   - 원본이 직접적 명령/질문이면 → 따라읽기 평가
